...
# Datos de ejemplo para demostrar el índice invertido
documentos = {
    1: "Los valores, principios y normas descritos en el Manual Ético de BSK Motorcycle Team son fundamentales"
       "para BSK Motorcycle Team. Este documento tiene la finalidad de permitir a todas las partes interesadas"
       "de BSK Motorcycle Team comprender claramente estos valores, principios y normas. El documento clave de"
       "este es el manual ético de BSK Motorcycle Team, el cual describe nuestros compromisos en cuatro áreas"
       "clave: dignidad, integridad, conducta, y confidencialidad. El Comité De Ética y el responsable de"
       "cumplimiento normativo tienen la obligación de garantizar la aplicación del manual ético de BSK Motorcycle"
       "Team y de los demás documentos y herramientas previamente establecidas por los órganos de BSK Motorcycle"
       "Team. No obstante, la responsabilidad última de respetarlos recae en cada uno de nosotros: es nuestra"
       "responsabilidad común comportarnos de manera ética y además garantizar que los demás hagan lo mismo."
       "Si se encontrase con situaciones no reflejadas en el manual ético de BSK Motorcycle Team, o si tuviese"
       "preguntas relativas a las cuestiones tratadas en este documento, deberá consultar sin falta al Comité "
       "De Ética o al responsable de cumplimiento normativo. Animo a todas las personas a las que afecta este"
       "documento a que tomen la iniciativa, dando ejemplo y promuevan el comportamiento ético en el marco de sus"
       "grupos y equipos.",
    2: "La programación en Python es clave para el trabajo con datos"
    ...
    3: "Un tercer documento para el índice invertido",
}
...
for doc_id, contenido in documentos.items():
    ...
    for palabra in palabras:
        ...
        if palabra in indice_invertido:
            ...
            indice_invertido[palabra] = [doc_id]

@app.get("/")
def read_root():
    return {"message": "¡Bienvenido al microservicio de índice invertido!"}

@app.get("/buscar/{palabra}")
def buscar_palabra(palabra: str):
    palabra = palabra.lower()
    if palabra in indice_invertido:
        return {"palabra": palabra, "documentos": indice_invertido[palabra]}
    else:
        return {"palabra": palabra, "documentos": []}
